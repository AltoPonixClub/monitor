    ...
    for (const auto &i : Configs::Vision::kBoardArucoPts) {
        for (auto c : i) {
            transform_src.emplace_back(
                c.x * Configs::Vision::kImgSize.width /
                    Configs::Physical::kPlatformDim.width,
                c.y * Configs::Vision::kImgSize.height /
                    Configs::Physical::kPlatformDim
                        .height); // Multiplier to make bigger in final image
        }
    }
...
void Vision::read(State *state) {
    ...
    if (state->detectedArucoIds.size() > 1) {
        ...
        spdlog::info("Vision: Rvec: {}, Tvec: {} {} {}", *state->camRvec.val,
                     state->camTvec.val[0], state->camTvec.val[1],
                     state->camTvec.val[2]);
        // Sets up transform dst for findHomography
        for (int i = 0; i < state->detectedArucoIds.size(); i++)
            tmp_corners.emplace_back(state->detectedArucoIds[i],
                                     state->detectedArucoCorners[i]);
    ...
    for (const auto &corner : state->detectedArucoCorners) {
        for (auto pt : corner) {
            cv::circle(outputs->editedCapFrame, pt,
                       ...
                       Configs::Display::kAqua, cv::FILLED);
        }
    }
}
