...
void Vision::configure() {
    ...
    for (const auto &i: constants::vision::boardArucoPts) {
        for (auto c: i) {
            transform_src.push_back(cv::Point2f(c.x * constants::vision::imgSize.width / constants::vision::platformDim.width,
...
void Vision::read(RobotState *state) {
    ...
    cv::aruco::refineDetectedMarkers(frame, board, state->detectedArucoCorners, state->detectedArucoIds, state->rejectedArucoCorners,
                                     cameraMatrix, distCoeffs);

    state->transform_dst.clear();
    if (state->detectedArucoIds.size() == 4) {
        std::vector<std::pair<int, std::vector<cv::Point2f>>> tmp_corners;
        cv::aruco::estimatePoseBoard(state->detectedArucoCorners, state->detectedArucoIds, board, cameraMatrix, distCoeffs, state->cam_rvec,
                                     state->cam_tvec);
        ...
        try {
            ...
            cv::warpPerspective(state->capFrame, state->undistortedFrame, H.inv(), constants::vision::imgDispSize, cv::INTER_LINEAR);
            state->undistortedFrame = state->undistortedFrame(cv::Rect(0, 0, constants::vision::imgSize.width, constants::vision::imgSize.height));
        }
        catch (...) {
