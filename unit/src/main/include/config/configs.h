...
public:
    ...
    public:
        ...
        static inline const std::string kCalibPath = std::string("/home/aoberai/programming/altoponix/monitor/unit/outputs.yml");
        ...
        static inline const cv::Ptr<cv::aruco::Dictionary> kArucoDictionary = cv::aruco::Dictionary::get(cv::aruco::DICT_5X5_100);
        static inline const cv::Ptr<cv::aruco::DetectorParameters> kArucoParams = cv::aruco::DetectorParameters::create();
        static inline const std::vector<int> kArucoIds = {24, 42, 48, 69};
        static inline const cv::Ptr<cv::aruco::Board> kBoard = cv::aruco::Board::create(kBoardArucoPts, kArucoDictionary, kArucoIds);
    };
    class Display {
    public:
